...
CREATE TABLE `member` (
  ...
  `nick_name` varchar(20) COLLATE utf8_bin NOT NULL COMMENT '회원 별명',
  `grade` varchar(10) COLLATE utf8_bin NOT NULL COMMENT '회원 등급',
  `updated_at` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '회원 정보 수정일시',
  `registed_at` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '회원 등록일시',
  PRIMARY KEY (`id`),
